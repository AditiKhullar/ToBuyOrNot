...
if __name__ == '__main__':
	...
	if len(sys.argv) == 2 and sys.argv[1]:
		if len(sys.argv[1]) == 40:
			...
			f.write(sys.argv[1])
			f.close()
			print('Key: ' + sys.argv[1] + ' was written to AlchemyAPI/api_key.txt')
...
class AlchemyAPI:
	...
	def __init__(self):
		...
		It loads the API key from AlchemyAPI/api_key.txt and configures the endpoints.
		"""
		...
		try:
			...
			if key == '':
				...
				print('The AlchemyAPI/api_key.txt file appears to be blank, please run: python alchemyapi.py YOUR_KEY_HERE')
				...
				print('It appears that the key in AlchemyAPI/api_key.txt is invalid. Please make sure the file only includes the API key, and it is the correct one.')
				sys.exit(0)
			else:
				#setup the key
				self.apikey = key
